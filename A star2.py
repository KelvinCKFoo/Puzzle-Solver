...
def heuristic(state):
    # Manhattan distance from the target block to the goal position
    target_pos = state[0]
    goal_pos = (100, 300)
...
def get_neighbors(state):
    ...
    # Loop through each block in the current state
    for i in range(len(state)):
        ...
        if new_x >= 0:  # Check if the move is within bounds
        ...
        # Move right
        new_x, new_y = x + BLOCK_SIZE, y
        if new_x < SCREEN_WIDTH:  # Check if the move is within bounds
            new_state = list(state)
            new_state[i] = (new_x, new_y)
            if not check_collision(new_state, i):
                neighbors.append(tuple(new_state))
        ...
        if new_y >= 0:  # Check if the move is within bounds
            new_state = list(state)
            new_state[i] = (new_x, new_y)
            if not check_collision(new_state, i):
                neighbors.append(tuple(new_state))
        ...
        # Move down
        new_x, new_y = x, y + BLOCK_SIZE
        if new_y < SCREEN_HEIGHT:  # Check if the move is within bounds
            new_state = list(state)
            new_state[i] = (new_x, new_y)
            if not check_collision(new_state, i):
                neighbors.append(tuple(new_state))
...
def dfs_solve(state, depth_limit, visited):
    ...
    while stack:
        ...
        # Sort neighbors based on heuristic (Manhattan distance)
